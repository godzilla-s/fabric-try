...
function createOrdereSecret() {
    ...
    for ((i=0; i<ordererNum; i++)); do 
        ...
        kubectl create secret tls hlf-${ordererOrg}-orderer${i}-tls -n $namespace --key $ordererTlsKey --cert $ordererTlsCert
        kubectl create secret generic hlf-${ordererOrg}-orderer${i}-tlsca-cert -n $namespace --from-file=ca.crt=$ordererTlsCa
    done 
